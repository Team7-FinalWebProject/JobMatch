...
def mysql_io(io, binary, dump):
    ...
    if io == 'import':
        ...
        print('DB imported successfully' if not process.stderr or not process.stderr.strip() else process.stderr)
    elif io == 'dump':
...
def main(silent_export = False, silent_import = False):
    ...
    if silent_import:
        ...
        copy = True
        if user_query(f'Would you like to export the {_SCHEMA} schema (Y/N)?').upper() == 'Y':
            dump()
        if user_query(f'Would you like to drop and import the {_SCHEMA} schema (Y/N)?').upper() == 'Y':
            drop()
            imp(copy = False)
        if user_query(f'Would you like additional commands (Y/N)?').upper() == 'Y':
