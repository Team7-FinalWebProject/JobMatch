...
def view_unapproved_professional(id: int, x_token: str = Header()):
    admin = admin_or_error(x_token)
    return search_service.admin_get_unapproved_professional_by_id(id) if admin.__class__.__name__ == 'Admin' else None

@admin_router.get('/professionals', tags=["Admin"])
def view_unapproved_professionals(x_token: str = Header()):
    admin = admin_or_error(x_token)
    return search_service.admin_get_unapproved_professionals() if admin.__class__.__name__ == 'Admin' else None
